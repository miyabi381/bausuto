...
// テキストオブジェクト
function ScoreEntity(val, x, y) {
    this.shape = ShapeScore;
...
// 敵オブジェクト
function RectEnemy(shape,x, y, width, height) {
    this.shape = shape;
    this.type = BodyStatic;
    this.x = x;
    this.y = y;
...
// Non_Rect
function Non_Rect(x, y, width, height) {
    this.shape = Non_Entity;
    this.type = Non_Body;
    this.x = x;
    this.y = y;
    this.w = width;
...
function CircleEntity(x, y, radius, type, restitution, deceleration) {
    ...
    this.collEnemy_1 = function (r) {  // 円と矩形の衝突 // 敵
    ...
    this.collEnemy_2 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_3 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_4 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_5 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_6 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_7 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_8 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_9 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collEnemy_10 = function (r) {  // 円と矩形の衝突 // 敵
        // 矩形の４辺上で最も円に近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
    ...
    this.collidedWithLine = function (line) {  // 円と線の衝突
        ...
        if (crossed) {
            ...
            var vec0 = line.norm.mul(-100 * dot0);
    ...
    this.step = function (elapsed) {
        ...
        entities.forEach(function (e) {
            if (e.type == BodyDynamic) {
                ...
                e.move(e.velocity.x*-1, e.velocity.y*-1);
            }
        });
            ...
            case 0:
                for (var i = 0; i < entities.length - 1; i++) {
                    for (var j = i + 1; j < entities.length; j++) {
                        ...
                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_1) {
                            e0.collEnemy_1(e1);

                        } else if (e0.shape == Enemy_1 && e1.shape == ShapeCircle) {
                            e1.collEnemy_1(e0);

                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_2) {
                            e0.collEnemy_2(e1);
                        } else if (e0.shape == Enemy_2 && e1.shape == ShapeCircle) {
                            e1.collEnemy_2(e0);
                        }
                    }
                }
            ...
            case 1:
                for (var i = 0; i < entities.length - 1; i++) {
                    for (var j = i + 1; j < entities.length; j++) {
                        ...
                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_3) {
                            e0.collEnemy_3(e1);
                        } else if (e0.shape == Enemy_3 && e1.shape == ShapeCircle) {
                            e1.collEnemy_3(e0);

                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_4) {
                            e0.collEnemy_4(e1);
                        } else if (e0.shape == Enemy_4 && e1.shape == ShapeCircle) {
                            e1.collEnemy_4(e0);

                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_5) {
                            e0.collEnemy_5(e1);
                        } else if (e0.shape == Enemy_5 && e1.shape == ShapeCircle) {
                            e1.collEnemy_5(e0);
                        }
                    }
                }
            ...
            case 2:
                for (var i = 0; i < entities.length - 1; i++) {
                    for (var j = i + 1; j < entities.length; j++) {
                        ...
                        } else if (e0.shape == ShapeCircle && e1.shape == Enemy_6) {
                            e0.collEnemy_6(e1);
                        } else if (e0.shape == Enemy_6 && e1.shape == ShapeCircle) {
                            e1.collEnemy_6(e0);
                        }
                        else if (e0.shape == ShapeCircle && e1.shape == Enemy_7) {
                            e0.collEnemy_7(e1);
                        } else if (e0.shape == Enemy_7 && e1.shape == ShapeCircle) {
                            e1.collEnemy_7(e0);
                        }
                        else if (e0.shape == ShapeCircle && e1.shape == Enemy_8) {
                            e0.collEnemy_8(e1);
                        } else if (e0.shape == Enemy_8 && e1.shape == ShapeCircle) {
                            e1.collEnemy_8(e0);
                        }
                        else if (e0.shape == ShapeCircle && e1.shape == Enemy_9) {
                            e0.collEnemy_9(e1);
                        } else if (e0.shape == Enemy_9 && e1.shape == ShapeCircle) {
                            e1.collEnemy_9(e0);
                        }
                        else if (e0.shape == ShapeCircle && e1.shape == Enemy_10) {
                            e0.collEnemy_10(e1);
                        } else if (e0.shape == Enemy_10 && e1.shape == ShapeCircle) {
                            e1.collEnemy_10(e0);
                        }
                    }
                }
